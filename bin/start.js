...
`);
    ...
    function flagsForBuildOptions(callName, options, isTTY2, logLevelDefault, writeDefault) {
      ...
      let write2 = (_a2 = getFlag(options, keys, "write", mustBeBoolean)) != null ? _a2 : writeDefault;
    ...
    function buildOrContextImpl(callName, buildKey, sendRequest, sendResponse, refs, streamIn, requestCallbacks, options, isTTY2, defaultWD2, callback) {
      ...
      function buildOrContextContinue(requestPlugins, runOnEndCallbacks, scheduleOnDisposeCallbacks) {
        ...
        if (write2 && !streamIn.hasFS) throw new Error(`The "write" option is unavailable in this environment`);
        const request = {
          command: "build",
          key: buildKey,
          entries,
      ...
      if (path5.basename(nodeModulesDirectory) === "node_modules") {
        for (const unixKey in knownUnixlikePackages) {
          try {
            ...
            if (fs.existsSync(path5.join(nodeModulesDirectory, pkg))) return pkg;
          } catch {
          }
        }
      ...
      if (/\.zip\//.test(binPath)) {
        ...
        if (pnpapi) {
          ...
          const binTargetPath = path5.join(
            ...
            ".cache",
            "esbuild",
            `pnpapi-${pkg.replace("/", "-")}-${"0.24.0"}-${path5.basename(subpath)}`
          );
          if (!fs.existsSync(binTargetPath)) {
            fs.mkdirSync(path5.dirname(binTargetPath), { recursive: true });
...
More information: The file containing the code for esbuild's JavaScript API (${__filename}) does not appear to be inside the esbuild package on the file system, which usually means that the esbuild package was bundled into another file. This is problematic because the API needs to run a binary executable inside the esbuild package which is located using a relative path from the API code to the executable. If the esbuild package is bundled, the relative path will be incorrect and the executable won't be found.`
    ...
    var randomFileName = () => {
      return path22.join(os2.tmpdir(), `esbuild-${crypto2.randomBytes(32).toString("hex")}`);
    };
...
import { cp as cp2, rm as rm2, mkdir as mkdir2 } from "node:fs/promises";
import path4 from "node:path";

// tasks/lib/esbuild-plugins.ts
import path3 from "node:path";
import { cp, readFile, writeFile as writeFile2 } from "node:fs/promises";
...
function getHashOfFile(path5) {
  return new Promise((resolve, reject) => {
    const hash = crypto.createHash("sha1");
    const rs = createReadStream(path5);
    rs.on("error", reject);
    rs.on("data", (chunk) => hash.update(chunk));
    rs.on("end", () => resolve(hash.digest("hex").slice(0, 8)));
  });
}
async function addHash(filename) {
...
async function glob(globString, targetDirectory2) {
  let files = [];
  for await (const file of Glob(globString, { cwd: targetDirectory2 })) {
    files.push(file);
  }
  return files;
}
async function findHashedFile(filename, ext, targetDirectory2) {
  let parsed = path.parse(filename);
  for await (const file of Glob(`**/${parsed.dir}/${parsed.name}.*${ext}`, { cwd: targetDirectory2 })) {
    return file;
  }
}
...
// tasks/lib/file-mapper.ts
import { rm } from "node:fs/promises";
import path2 from "node:path";
function isFileMapper(x) {
  return x?.url && x?.file;
}
var option = {
  isRunning: false,
...
async function fileMapper(targetDirectory2, force = false) {
  if (option.isRunning || option.isWaiting || !force) {
    ...
    option.isWaiting = setTimeout(() => {
      ...
      fileMapper(targetDirectory2, true);
    }, 30);
    return;
  }
  try {
    ...
    let oldFileMapperFiles = await glob("**/file-map.*.js", targetDirectory2);
    Promise.all(oldFileMapperFiles.map(async (x) => {
      await rm(`${targetDirectory2}/${x}`);
    }));
    let files = await glob("**/*.{js,css,json,ico,svg,png}", targetDirectory2);
    let mapper = files.map((x) => {
      let parsed = path2.parse(x);
      let parsed2 = path2.parse(parsed.name);
    ...
    await write(`${targetDirectory2}${fileMapUrl}`, fileMapContent);
    let globalFiles = mapper.filter((x) => x.url.includes(".global.")).map((x) => x.file).join(`","`);
    let swFile = mapper.find((x) => x.url === "/web/sw.js")?.file;
    let globals = `${globalFiles && `"`}${globalFiles}${globalFiles && `",`}`;
    if (option.isWaiting) return;
    await write(
      `${targetDirectory2}/web/sw.js`,
...
// tasks/lib/esbuild-plugins.ts
function transformExportToReturn(targetDirectory2) {
  return {
    ...
    setup(build) {
      build.onEnd(async (result) => {
        ...
        await setPagesToReturn(targetDirectory2);
      });
    }
  };
}
async function setPagesToReturn(targetDirectory2) {
  let files = await glob("**/*.js", targetDirectory2);
  await Promise.all(files.map(async (file) => {
    let filename = path3.join(targetDirectory2, file);
    ...
    if (matched) {
      ...
      let matched2 = content.match(/var (\w*) = (\w*);\sexport \{\s.*\s.*/);
      if (!matched2) {
        return;
      }
      content = content.replace(matched2[0], `return ${matched2[2]};`);
    }
    await writeFile2(filename, content, "utf-8");
  }));
}
function updateFileMapper(targetDirectory2) {
  return {
    ...
    setup(build) {
      build.onEnd(async (result) => {
        ...
        await fileMapper(targetDirectory2);
      });
    }
  };
}
function updateHTML(targetDirectory2) {
  return {
    ...
    setup(build) {
      build.onEnd(async (result) => {
        ...
        await handleHTML(targetDirectory2);
      });
    }
  };
}
async function handleHTML(targetDirectory2) {
  console.time("Copying HTML");
  await cp(
    "./node_modules/@jon49/sw/lib/app-loader.html",
    `${targetDirectory2}/web/index.html`,
    { recursive: true }
  );
  let files = await glob("**/*.html", "./src");
  let mappableFiles = (await Promise.all(
    (await glob("**/{css,js,images}/*.*", "./src")).map(async (x) => {
      let ext = path3.extname(x);
      if (ext === ".ts") {
        ext = ".js";
      }
      let hashedFile = await findHashedFile(x, ext, targetDirectory2);
      if (!hashedFile) {
        return;
      }
      return {
  ...
  await Promise.all(files.map(async (filename) => {
    ...
    await write(`${targetDirectory2}/${filename}`, content);
  }));
...
var Arguments = class {
  constructor() {
    this.argv = (0, import_minimist.default)(process.argv.slice(2));
  }
  get _() {
    return this.argv._;
  }
  ...
  get port() {
    return this.argv.p || this.argv.port || 3e3;
  }
  get targetDirectory() {
...
var targetDirectory = argv.targetDirectory;
var isProd = argv.isProd;
console.time("Cleaning");
await rm2(targetDirectory, { recursive: true, force: true });
console.timeEnd("Cleaning");
console.time("Copying Files");
await mkdir2(targetDirectory, { recursive: true });
var filesToCopy = await glob("**/*.{ico,png,svg,json}", "./src");
var copyFiles = await glob("**/*.min.*", "./src");
filesToCopy.push(...copyFiles);
var hashedFiles = await Promise.all(filesToCopy.map((x) => addHash(x)));
await Promise.all(filesToCopy.map((filename, i) => {
  let source = path4.join("src", filename);
  let target = path4.join(targetDirectory, hashedFiles[i]);
  return cp2(source, target, { recursive: true });
}));
console.timeEnd("Copying Files");
console.time("Processing JS");
var ctxBundles = await import_esbuild.default.context({
  entryPoints: [
    "./src/**/*.bundle.ts"
  ],
...
var ctxIIFEs = await import_esbuild.default.context({
  entryPoints: [
    "./src/**/*.global.ts",
    "./src/web/sw.ts"
  ],
...
var staticFiles = await glob("**/js/*.{js,ts}", "./src");
var isStaticFile = /.*\/[0-9a-zA-Z\-]+.[jt]s/;
var staticEntryPoints = staticFiles.filter((x) => isStaticFile.test(x) && !x.includes(".bundle.") && !x.includes(".min.") && !x.includes("sw.ts")).map((x) => `./src/${x}`);
var ctxStaticFiles = await import_esbuild.default.context({
  entryPoints: [
    "./src/**/*.css",
    ...staticEntryPoints
  ],
...
var ctxPages = await import_esbuild.default.context({
  entryPoints: [
    "./src/**/*.page.ts"
  ],
  entryNames: "[dir]/[name].[hash]",
